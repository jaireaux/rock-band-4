...
#back-to-chart{position:fixed;left:50%;bottom:calc(18px + env(safe-area-inset-bottom,0px));transform:translateX(-50%);z-index:5;width:52px;height:52px;border:1px solid #ffffff66;border-radius:50%;background:var(--purple);color:white;font-size:30px;line-height:1;box-shadow:0 6px 24px #17152d40}#back-to-chart:hover{background:#5546d6}#back-to-chart[hidden]{display:none}
</style></head><body>
...
<button id="back-to-chart" type="button" aria-label="Back to top of chart" title="Back to top of chart" hidden><span aria-hidden="true">↑</span></button>
...
const backToChart=document.getElementById('back-to-chart');
function updateBackToChart(){
 const firstRow=body.firstElementChild;
 const headerBottom=document.querySelector('thead').getBoundingClientRect().bottom;
 backToChart.hidden=!firstRow||firstRow.getBoundingClientRect().bottom>Math.max(0,headerBottom);
}
let scrollCheckPending=false;
function scheduleBackToChart(){
 if(scrollCheckPending)return;
 scrollCheckPending=true;
 requestAnimationFrame(()=>{scrollCheckPending=false;updateBackToChart();});
}
window.addEventListener('scroll',scheduleBackToChart,{passive:true});
window.addEventListener('resize',scheduleBackToChart);
backToChart.addEventListener('click',()=>{
 document.getElementById('chart-top').scrollIntoView({behavior:'instant',block:'start'});
 document.querySelector('[data-column]').focus({preventScroll:true});
 updateBackToChart();
});
